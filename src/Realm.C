  ...
  const stk::topology &theTopo)
{
  ...
  const double zero = 0;
  VectorFieldType& averageNormal = metaData_->declare_field<VectorFieldType>(stk::topology::NODE_RANK, "average_open_normal");
  stk::mesh::put_field(averageNormal, *part, nDim, &zero);

  const int zeroInt = 0 ;
  ScalarIntFieldType& faceConnectionCount = metaData_->declare_field<ScalarIntFieldType>(stk::topology::NODE_RANK, "open_face_connection_count");
