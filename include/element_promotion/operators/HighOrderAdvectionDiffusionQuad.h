  ...
  matrix_view<AlgTraitsQuad<poly_order>> lhs)
{
  ...
  for (int n = 0; n < n1D; ++n) {
    ...
    for (int j = 0; j < n1D; ++j) {
      ...
      ftype interp_term = 0.0;
      for (int k = 0; k < n1D; ++k) {
        interp_term += mat.nodalWeights(n, k) * mat.nodalDeriv(k, j) * diff_metric(XH, YH, m_minus, k);
      }
      interp_term -= mat.nodalWeights(m_minus, j) * adv_metric(XH, m_minus, j);
    ...
    for (int m = 1; m < n1D - 1; ++m) {
      for (int j = 0; j < n1D; ++j) {
        ...
        ftype interp_term_m1 = 0.0;
        ftype interp_term_p0 = 0.0;
        for (int k = 0; k < n1D; ++k) {
        ...
        interp_term_m1 -= w * adv_metric(XH, m - 1, j);
        interp_term_p0 -= w * adv_metric(XH, m + 0, j);
    ...
    for (int j = 0; j < n1D; ++j) {
      ...
      interp_term -= mat.nodalWeights(n,j) * adv_metric(XH, m_plus-1, j);
  ...
  for (int m = 0; m < n1D; ++m) {
    ...
    for (int i = 0; i < n1D; ++i) {
      ...
      interp_term -= mat.nodalWeights(m, i) * adv_metric(YH, n_minus, i);
    ...
    for (int n = 1; n < n1D - 1; ++n) {
      for (int i = 0; i < n1D; ++i) {
        ...
        interp_term_m1 -= w * adv_metric(YH, n - 1, i);
        interp_term_p0 -= w * adv_metric(YH, n + 0, i);
    ...
    for (int i = 0; i < n1D; ++i) {
      ...
      interp_term -=  mat.nodalWeights(m, i) * adv_metric(YH, n_plus - 1, i);
